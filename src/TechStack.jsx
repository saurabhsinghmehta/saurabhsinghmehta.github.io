...
const TechStack = () => {
    ...
    const color = useColorModeValue('brown')
    const imagesYellow = [
        {
    ...
    return (
        <Box paddingY={"40px"} paddingTop={"80px"} bg={bg} color={color} height={"max-content"} id="techStack">
            <Text textAlign={"center"} fontSize="5xl" color={"white"} paddingY={"20px"}>
                Tech <a color={"white"}>Stack</a>
            </Text>
            <Fade left cascade>
                <Box
                    width="60%"
                    ...
                    {(bg === 'white' ? imagesBlue : imagesYellow).map((elem) => {
                        return <Box className='hvr-pop'
                            ...
                            box-shadow="rgba(0, 0, 0, 0.35) 0px 5px 15px" _hover={{ borderColor: "white" }}>
                            <ImageWrapper>
